...
import com.bahar.repository.mapper.MtUserTagRuleMapper;
...
public class UserTagRuleServiceImpl extends ServiceImpl<MtUserTagRuleMapper, MtUserTagRule> implements UserTagRuleService {
    ...
    public void executeRulesForUser(MtUser user, MtOrder order, Integer ruleId, AccountInfo accountInfo) {
        ...
        for (MtUserTagRule rule : rules) {
            try {
                ...
                if (isMatch) {
                    ...
                    log.debug("会员[{}]不再符合规则[{}]，标签[{}]将被移除", user.getId(), rule.getRuleName(), rule.getTagId());
                }
            } catch (Exception e) {
        ...
        if (!newSet.equals(existSet)) {
            ...
            log.info("会员[{}]标签已更新，原标签: {} -> 新标签: {}", user.getId(), existTagIds, newTagIds);
        }
    }
