      ...
      <view class="widget-list">
        <view class="widget-detail" v-for="(item, index) in list.content" :key="index">
          <view class="row-block dis-flex flex-y-center">
            <view class="order">订单号：{{ item.orderInfo.orderSn }}</view>
          ...
          <view class="detail-order row-block">
            <view class="item dis-flex flex-x-end flex-y-center">
              <view class="flex-box">售后时间：{{ item.createTime }}</view>
          ...
          <view class="detail-operate row-block dis-flex flex-x-end flex-y-center">
            <view class="btn-detail" @click.stop="handleTargetDetail(item.id)">详情</view>
            <view class="btn-confirm" @click="handleConfirm(item.id)">更新</view>
          </view>
        </view>
      </view>
    </mescroll-body>
...
<script>
  ...
  export default {
     ...
     */
    onLoad(options) {
       this.onChangeTab(options.status ? options.status : 0)
    },
    ...
    methods: {
      ...
      getRefundList(pageNo = 1) {
        ...
        return new Promise((resolve, reject) => {
          RefundApi.list({ status: app.getTabValue(), keyword: app.keyword, page: pageNo }, { load: false })
            .then(result => {
              // 合并新数据
      ...
      // 跳转到审核
      handleConfirm(refundId) {
        this.$navTo('merchantPages/refund/update', { refundId })
      }
    }
  }
</script>
